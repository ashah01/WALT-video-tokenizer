...
class VideoTokenizerTrainer:
        ...
        *,
        ...
        if not exists(dataset):
            ...
            dataset = dataset_klass(
                dataset_folder, image_size=model.image_size, **dataset_kwargs
            )
        ...
        self.valid_dataloader = DataLoader(
            valid_dataset, shuffle=True, drop_last=True, batch_size=batch_size
        )
        ...
        self.discr_optimizer = get_optimizer(
            model.discr_parameters(), lr=learning_rate, **optimizer_kwargs
        )
        ...
        self.discr_warmup = warmup.LinearWarmup(
            self.discr_optimizer, warmup_period=warmup_steps
        )
    ...
    def train_step(self, dl_iter):
        ...
        for grad_accum_step in range(self.grad_accum_every):
            ...
            context = (
                partial(self.accelerator.no_sync, self.model)
                if not is_last
                else nullcontext
            )
        ...
        if exists(self.max_grad_norm):
            self.accelerator.clip_grad_norm_(
                self.model.discr_parameters(), self.max_grad_norm
            )

            if self.has_multiscale_discrs:
                for multiscale_discr in self.model.multiscale_discrs:
                    self.accelerator.clip_grad_norm_(
                        multiscale_discr.parameters(), self.max_grad_norm
                    )
    ...
    def train(self):
        ...
        while step < self.num_train_steps:
            ...
            if self.is_main and not (step % self.checkpoint_every_step):
                ...
                checkpoint_path = (
                    self.checkpoints_folder / f"checkpoint.pt" # modify convention to save disk space
                )
